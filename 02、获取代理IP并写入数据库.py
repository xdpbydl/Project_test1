...
if __name__ == '__main__':
    ...
    #如果平均时间超过200ms重新选取ip
    for i in range(len(proxys_list)):
        ...
        if average_time > 200:
            ...
            print(proxy + "ip连接超时, 重新获取中!")
        if average_time < 200:
            proxy_dict = {split_proxy[0]: split_proxy[1] + ':' + split_proxy[2]}

            sql = "insert into ProxyIP(ip,prot,type,connected,createtime) values('" + split_proxy[1] + "','" + split_proxy[2] + "','" + split_proxy[0] + "','" + '1' + "','" + nowTime + "')"
            print(sql)
            try:
                cur.execute (sql)
            except:
